...
apiClient_t *apiClient_create() {
    ...
    apiClient->basePath = strdup("https://api.api2cart.local.com/v1.1");
    ...
    apiClient->apiKeys_StoreKeyAuth = NULL;
...
, list_t *apiKeys_StoreKeyAuth
    ...
    if(apiKeys_StoreKeyAuth!= NULL) {
        apiClient->apiKeys_StoreKeyAuth = list_createList();
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiKeys_StoreKeyAuth) {
            keyValuePair_t *pair = listEntry->data;
            keyValuePair_t *pairDup = keyValuePair_create(strdup(pair->key), strdup(pair->value));
            list_addElement(apiClient->apiKeys_StoreKeyAuth, pairDup);
        }
    }else{
        apiClient->apiKeys_StoreKeyAuth = NULL;
    }
...
void apiClient_free(apiClient_t *apiClient) {
    ...
    if(apiClient->apiKeys_StoreKeyAuth) {
        listEntry_t *listEntry = NULL;
        list_ForEach(listEntry, apiClient->apiKeys_StoreKeyAuth) {
            keyValuePair_t *pair = listEntry->data;
            if(pair->key){
                free(pair->key);
            }
        ...
        list_freeList(apiClient->apiKeys_StoreKeyAuth);
    }
    free(apiClient);
}
...
void apiClient_invoke(apiClient_t    *apiClient,
    ...
    if(handle) {
        ...
        // this would only be generated for apiKey authentication
        if (apiClient->apiKeys_StoreKeyAuth != NULL)
        {
        list_ForEach(listEntry, apiClient->apiKeys_StoreKeyAuth) {
        keyValuePair_t *apiKey = listEntry->data;
        if((apiKey->key != NULL) &&
