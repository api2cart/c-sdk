...
//
account_cart_list_200_response_t* AccountAPI_accountCartList(apiClient_t *apiClient, char *params, char *exclude, char *request_from_date, char *request_to_date, char *store_url, char *store_key);
```
...
//
account_failed_webhooks_200_response_t* AccountAPI_accountFailedWebhooks(apiClient_t *apiClient, int *count, int *start, char *ids);
```
