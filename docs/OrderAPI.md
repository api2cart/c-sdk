...
//
order_count_200_response_t* OrderAPI_orderCount(apiClient_t *apiClient, char *customer_id, char *customer_email, char *order_status, list_t *order_status_ids, char *created_to, char *created_from, char *modified_to, char *modified_from, char *store_id, char *ids, char *order_ids, char *ebay_order_status, char *financial_status, list_t *financial_status_ids, char *fulfillment_channel, char *fulfillment_status, char *shipping_method, char *delivery_method, char *tags, char *ship_node_type);
```
...
//
order_find_200_response_t* OrderAPI_orderFind(apiClient_t *apiClient, char *customer_id, char *customer_email, char *order_status, int *start, int *count, char *params, char *exclude, char *created_to, char *created_from, char *modified_to, char *modified_from, char *financial_status);
```
...
//
order_info_200_response_t* OrderAPI_orderInfo(apiClient_t *apiClient, char *order_id, char *id, char *params, char *response_fields, char *exclude, char *store_id, int *enable_cache, int *use_latest_api_version);
```
...
//
order_shipment_info_200_response_t* OrderAPI_orderShipmentInfo(apiClient_t *apiClient, char *id, char *order_id, int *start, char *params, char *response_fields, char *exclude, char *store_id);
```
...
//
model_response_order_shipment_list_t* OrderAPI_orderShipmentList(apiClient_t *apiClient, char *order_id, char *page_cursor, int *start, int *count, char *params, char *response_fields, char *exclude, char *created_from, char *created_to, char *modified_from, char *modified_to, char *store_id);
```
...
//
model_response_order_transaction_list_t* OrderAPI_orderTransactionList(apiClient_t *apiClient, char *order_ids, int *count, char *store_id, char *params, char *response_fields, char *exclude, char *page_cursor);
```
